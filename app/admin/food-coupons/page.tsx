...
export default function AdminFoodCouponsPage(){
  ...
  const getCouponsForTeam = (teamName: string): any[] => {
    ...
    try {
      const keyEnc = `foodCoupons_${encodeURIComponent(teamName)}`;
      const keyPlain = `foodCoupons_${teamName}`;
  ...
  const getTeamMealFilterValue = (teamName: string, meal: 'Dinner' | 'Lunch'): string => {
    return getMealRedeemedCountForTeam(teamName, meal) > 0 ? 'Redeemed' : 'Not redeemed';
  };

  const getMemberMealStatus = (teamName: string, memberKey: string, meal: 'Dinner' | 'Lunch'): string => {
    const arr = getCouponsForTeam(teamName);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-md border-2 border-gitam-300 p-6 mb-6">
          <div className="flex justify-between items-center">
            <h1 className="text-3xl font-bold text-gitam-700">Food Coupons - Admin</h1>
            <button
        ...
        {tab==='teams' && (
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-gitam-100 border-b-2 border-gitam-300">
                    ...
                    <th className="p-3 text-left font-semibold text-gitam-700 border-r border-gitam-200">Dinner Redeemed</th>
                    <th className="p-3 text-left font-semibold text-gitam-700 border-r border-gitam-200">Lunch Redeemed</th>
                ...
                <tbody>
                  {filteredTeams.map((t:any,i:number)=> (
                    <tr key={i} className="border-b border-gitam-200 odd:bg-white even:bg-gitam-50/40 hover:bg-gitam-100 transition-colors">
                      ...
                      <td className="p-3 border-r border-gitam-200 whitespace-nowrap">{getMealRedeemedCountForTeam(t.teamName, 'Dinner')}/{(t.members||[]).length}</td>
                      <td className="p-3 border-r border-gitam-200 whitespace-nowrap">{getMealRedeemedCountForTeam(t.teamName, 'Lunch')}/{(t.members||[]).length}</td>
          ...
          <div className="bg-white rounded-xl shadow-lg border-2 border-gitam-300 p-6">
            <div className="mb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-8 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium text-gitam-700 mb-1">Dinner</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gitam-700 mb-1">Lunch</label>
      ...
      {selectedTeam && teamCoupons && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-40">
          <div className="w-full max-w-3xl bg-white rounded-xl shadow-2xl border-2 border-gitam-300 p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gitam-700">{selectedTeam.teamName} — Food Coupons</h3>
              <div className="flex gap-2">
